            ...
            <a href="{{ route('news.show', $banner->news->slug) }}" class="block">
                <div
                    ...
                    <div class="relative z-10 mb-3" style="padding-left: 10px;">
                        <div class="bg-primary text-white text-xs rounded-lg w-fit px-3 py-1 font-normal mt-3">{{ $banner->news->newsCategory->title }}
                        </div>
...
<div class="flex flex-col px-14 mt-10 ">
    ...
    <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-5">
        @foreach ($featured as $f)
        <a href="{{ route('news.show', $f->slug) }}">
            <div
                ...
                style="height: 100%">
                <span class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal ml-2 mt-2 text-sm absolute">
                    {{ $f->newsCategory->title }}
                </span>
                <img src="{{ asset('storage/' . $f->thumbnail) }}" alt="" class="w-full rounded-xl mb-3" style="height: 190px; object-fit: cover;">
                <p class="font-bold text-base mb-1">{{ $f->title }}</p>
                <p class="text-slate-400">{{ $f->created_at->diffForHumans() }}</p>
            </div>
        </a>
        @endforeach
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div
            ...
            <a href="{{ route('news.show', $news[0]->slug) }}">
                ...
                <img src="{{ asset('storage/' . $news[0]->thumbnail) }}" alt="berita1" class="rounded-2xl" style="width: 760px; object-fit: cover;">
                <p class="font-bold text-xl mt-3">
                    {{ $news[0]->title }}
                </p>
                <p class="text-slate-400 text-base mt-1">
                    {!! Str::markdown(Str::limit($news[0]->content, 100)) !!}
                </p>
                <p class="text-slate-400 text-base mt-1">{{ $news[0]->created_at->DiffForHumans() }}</p>
            </a>
        </div>
        ...
        <a href="{{ route('news.show', $new->slug) }}"
            class="relative col-span-5 flex flex-col h-fit sm:w-full md:flex-row gap-3 border border-slate-200 p-3 rounded-xl hover:border-primary hover:cursor-pointer">
            ...
            <img src="{{ asset('storage/' . $new->thumbnail) }}" alt="{{ $new->slug }}" class="rounded-xl w-full md:max-h-48" style="width: 320px; object-fit: cover;">
            <div class="mt-2 md:mt-0">
                <p class="font-semibold text-lg">{{ $new->title }}</p>
                <p class="text-slate-400 mt-3 text-sm font-normal">{!! Str::markdown(Str::limit($new->content, 100)) !!}</p>
            </div>
        </a>
        @endforeach
    </div>
...
<div class="flex flex-col px-14 mt-10 mb-10">
    ...
    <div class="grid sm:grid-cols-1 gap-5 lg:grid-cols-4">
        @foreach ($news as $nw)
        <a href="{{ route('news.show', $nw->slug) }}">
            <div
                ...
                <div class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal ml-2 mt-2 text-sm absolute">
                    {{ $nw->newsCategory->title }}</div>
                <img 
                    src="{{ asset('storage/' . $nw->thumbnail) }}" 
                    alt="{{ $nw->slug }}" 
                    class="w-full rounded-xl mb-3" 
                    style="height: 200px; object-fit: cover;">
                <p class="font-bold text-base mb-1">{{ $nw->title }}</p>
                <p class="text-slate-400">{{ $nw->created_at->DiffForHumans() }}</p>
            </div>
        </a>
        @endforeach
    </div>
