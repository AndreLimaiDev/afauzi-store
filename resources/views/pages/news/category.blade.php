...
<div class="w-full mb-16 bg-[#F6F6F6]">
    <h1 class="text-center font-bold text-2xl p-24">{{ $categories->title }}</h1>
</div>
        ...
        <a href="{{ route('news.show', $category->slug) }}">
            <div
                ...
                <div class="bg-primary text-white rounded-full w-fit px-5 py-1 font-normal ml-2 mt-2 text-sm absolute">
                    {{ $category->newsCategory->title }}</div>
                <img src="{{ asset('storage/' . $category->thumbnail) }}" alt="{{ $category->slug }}"
                    class="w-full rounded-xl mb-3" style="height: 300px; object-fit:cover;">
                <p class="font-bold text-base mb-1">{{ $category->title }}</p>
                <p class="text-slate-400">{{ $category->created_at->DiffForHumans() }}</p>
            </div>
        </a>
        @endforeach
    </div>
